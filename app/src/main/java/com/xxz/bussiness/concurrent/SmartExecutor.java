package com.xxz.bussiness.concurrent;


import com.xxz.magnet.utils.LogUtils;
import com.xxz.magnet.utils.MobileUtils;
...
public class SmartExecutor implements Executor {
    ...
    private static final int CPU_CORE = MobileUtils.getCoresNumbers();
    ...
    public void execute(final Runnable command) {
        ...
        if (callerRun) {
            LogUtils.v("SmartExecutor task running in caller thread");
            command.run();
        }
    }
    ...
    private void scheduleNext(WrappedRunnable scheduler) {
        synchronized (lock) {
            ...
            if (!suc) {
                runningList.clear();
                LogUtils.v("SmartExecutor scheduler remove failed, so clear all(running list) to avoid unpreditable error : " + scheduler);
            }
            if (waitingList.size() > 0) {
                ...
                if (waitingRun != null) {
                    ...
                    LogUtils.v("SmartExecutor get a NULL task from waiting queue: " + Thread.currentThread().getName());
                }
            } else {
    ...
    public void printThreadPoolInfo() {
        LogUtils.v("___________________________");
        LogUtils.v("state (shutdown - terminating - terminated): " + threadPool.isShutdown()
                + " - " + threadPool.isTerminating() + " - " + threadPool.isTerminated());
        LogUtils.v("pool size (core - max): " + threadPool.getCorePoolSize()
               + " - " + threadPool.getMaximumPoolSize());
        LogUtils.v("task (active - complete - total): " + threadPool.getActiveCount()
               + " - " + threadPool.getCompletedTaskCount() + " - " + threadPool.getTaskCount());
        LogUtils.v("waitingList size : " + threadPool.getQueue().size() + " , " + threadPool.getQueue());
    }
